...
def render() -> None:
        ...
        if uploaded_file is not None:
            try:
                ...
                if not trades:
                    ...
                    st.success(f"Analyzed {report.trade_analysis.total_trades} trades")
    ...
    with tab2:
        ...
        if stored_trades:
            ...
            pnls = [t.pnl_pct for t in stored_trades if t.action in ("SELL",)]
            if pnls:
                wins = sum(1 for p in pnls if p > 0)
                total = len(pnls)
                st.metric("Closed Trades Win Rate", f"{wins/total*100:.1f}%" if total else "0%")
        else:
