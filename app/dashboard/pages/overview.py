...
def render() -> None:
    ...
    with col1:
        st.metric("BTC/USDT", f"${latest_price:,.2f}", f"{change_24h:+.2f}%", border=True)

    with col2:
        if candles_1d:
            vol_avg = sum(c.volume for c in candles_1d[-7:]) / 7 if len(candles_1d) >= 7 else 0
            st.metric("Vol 7d (avg)", f"{vol_avg:,.0f}", border=True)
        else:
            st.metric("Vol 7d (avg)", "—", border=True)
    ...
    with col3:
        ...
        st.metric("Rango 30d", f"${low_30d:,.0f} - ${high_30d:,.0f}", f"{range_pct:.1f}%", border=True)

    with col4:
        tv = get_portfolio_value()
        st.metric("Valor Cartera", f"${tv:.2f}", border=True)
    ...
    with col_a:
        ...
        status_data = {
            "Modo": config.mode,
            "Kill Switch": "⚠️ Activo" if config.kill_switch else "✅ Inactivo",
            "Data DB": f"{len(candles_4h)} velas BTC 4h",
            "Última vela": str(candles_4h[-1].open_time) if candles_4h else "—",
    ...
    with col_b:
        st.subheader("Límites de Riesgo")
        try:
            with open("settings.yaml") as f:
                raw = yaml.safe_load(f) or {}
            risk = raw.get("risk", {})
            risk_data = {
                "Máx por trade": f"{risk.get('max_position_size_pct', 0.03)*100:.1f}%",
            ...
            risk_data = {"Máx por trade": "3.0%", "Riesgo por trade": "1.0%", "Stop loss": "2.0%"}
        for k, v in risk_data.items():
            st.text(f"{k}: {v}")

    with col_c:
        st.subheader("Resumen de Estrategias")
        st.info("Ejecuta backtests en la sección Backtesting.")
