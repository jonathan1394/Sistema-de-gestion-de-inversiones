...
def _buy_position(trade_symbol: str, trade_pct: int, current_price: float) -> None:
    try:
        ...
        logger.exception("Error in buy position")
        st.error("Error executing buy order.")
...
def _sell_position(trade_symbol: str, trade_pct: int, current_price: float) -> None:
    try:
        ...
        logger.exception("Error in sell position")
...
def _reset_portfolio() -> None:
    cap = load_settings().capital.initial_usdt
    st.session_state.portfolio_capital = cap
    st.session_state.portfolio_cash = cap
    st.session_state.portfolio_positions = {}
    st.session_state.portfolio_snapshots = []
    st.session_state.portfolio_peak = cap
...
def render() -> None:
    ...
    try:
        ...
        st.error("Error loading portfolio page.")
        st.stop()
