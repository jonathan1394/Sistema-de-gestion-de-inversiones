...
class DataAccessObject:
    ...
    def store_klines_chunked(
        self, symbol: str, interval: str, klines: list[list], chunk_size: int = 1000
    ) -> int:
    ...
    def add_prospect(self, symbol: str, interval: str = "1d", notes: str = "") -> dict[str, Any]:
    ...
    def update_prospect_status(self, symbol: str, interval: str, status: str) -> None:
        with connection_scope(self._db_path) as conn:
            conn.execute(
    ...
    def add_snapshot(self, total_value: float, cash: float, drawdown_pct: float = 0.0) -> None:
        now = datetime.now(timezone.utc).isoformat()
        with connection_scope(self._db_path) as conn:
            conn.execute(
